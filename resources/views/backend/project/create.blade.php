<x-backend.layout>
    <div>
        <x-form action="{{ route('bk-project-store') }}">
                ...
                <div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <x-select name="business_id">
                            ...
                            <option value="{{ $company->id }}" {{ old('business_id') == $company->id ? 'selected' : '' }}>
                                {{ $company->name }}</option>
                            @endforeach
                        </x-select>
                   ...
                   <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="p-2">
                            ...
                            <x-date-picker name="date_finish" value="{{ old('date_finish', $project->date_finish ?? '') }}"></x-date-picker>
                        </div>
                        <div class="p-2">
                            <x-label>
                                Sell Date <span class="fas fa-exclamation-circle"></span>
                            </x-label>
                            <x-date-picker name="date_sell" value="{{ old('date_sell', $project->date_sell ?? '') }}"></x-date-picker>
                        </div>
                    </div>
                   ...
                   <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="p-2">
                            <x-label>
                                <x-input-checkbox name="featured" value="{{ old('featured', $project->featured ?? '') }}" label="Featured" />
                            </x-label>
                        </div>
                   </div>
                   <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
