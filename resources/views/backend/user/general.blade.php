        ...
        <div class="grid lg:grid-cols-4">
            <div class="flex justify-center">
                ...
                @endif
                    ...
                    <div x-show="!edit" class="relative">
                        ...
                        <img src="{{ $user->avatar }}" class="w-36 h-36 rounded-full object-cover">
                    </div>
                    @endif
                    <div x-show="edit" class="relative">
                        <x-button-danger type="button" x-on:click="edit = false"
                            class="absolute cursor-pointer right-0 bottom-0 opacity-50 hover:opacity-100 z-50">
                            x
                        </x-button-danger>
                        <x-filepond class="w-36 h-36 -mt-2" name="avatar" avatar-style
                            uploadUrl="{{ url('api/temp-avatar-uploader') }}" file-size-validation="2MB"
                            file-type-validation="image/*" correct-image-orientation
            ...
            <div class="lg:col-span-3">
                ...
                <div class="p-2">
                    ...
                    <x-select name="primary_company">
                        ...
                        <option value="{{ $company->id }}"
                            {{ old('primary_company', $user->primary_company) == $company->id ? 'selected' : '' }}>
                            {{ $company->name }}</option>
                        @endforeach
                    </x-select>
                ...
                <div class="p-2">
                    ...
                    <x-multi-select placeholder="Select Groups" name="groups"
                        :selected="$selectedGroups" :unselected="$unselectedGroups" />
                </div>
                <div class="p-2">
                    <x-label>
                ...
                <div class="p-2">
                    <x-label>
                        <x-input-checkbox name="is_contact"
                            value="{{ $user->is_contact ?? old('is_contact') }}" label="Is Contact" />
                    </x-label>
                </div>
            </div>
        </div>
            ...
            <div x-data="{ on : false }">
                <form action="{{ route('bk-user-delete', $user) }}" method="POST"
                    ...
                    <x-confirm submitLabel="Delete User" form="delete-user-{{ $user->id }}">
                        <x-slot name="trigger">
                            <x-button-red x-on:click.prevent="on = ! on" class="whitespace-nowrap ml-4">
                                Delete
                                User</x-button-red>
                        </x-slot>
